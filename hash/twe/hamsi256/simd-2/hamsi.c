...
HashReturn InitHM
  (hashStateHM* const state, const int hashbitlen)
{
  __m128i c0, c1;
...
HashReturn UpdateHM
  (hashStateHM* const state, const BitSequence* data, DataLength databitlen)
{
  int i;
...
HashReturn FinalHM
  (hashStateHM* const state, BitSequence* const hashval)
{
  const int bufferbytes = (state->databitlen % 32) / 8;
...
{
    hashStateHM state;
    HashReturn ret = InitHM(&state, hashbitlen);

    if (ret == SUCCESS)
      {
        UpdateHM(&state, data, databitlen);
        FinalHM(&state, hashval);
      }
